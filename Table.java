...
public class Table{
	...
	public static int pages(RandomAccessFile file){
		...
		try 
		{
			num_of_pages = (int)(file.length()/(new Long(pageSize)));
		}catch(Exception e){
	...
	public static String[] getColName(String table){
		...
		try
		{
			RandomAccessFile file_pointer = new RandomAccessFile("data/davisbase_columns.tbl", "rw");
			...
			filter(file_pointer, table_name, columnName, buffer);
			HashMap<Integer, String[]> content_data = buffer.content;
			ArrayList<String> array = new ArrayList<String>();
			for(String[] i : content_data.values()){
				array.add(i[2]);
			}
			int size=array.size();
	...
	public static String[] getDataType(String table){
		...
		try
		{
			...
			filter(file_pointer, cmp, column_name, buffer);
			...
			dataType = array.toArray(new String[length]);
			file_pointer.close();
			return dataType;
		}catch(Exception e){
			System.out.println(e);
