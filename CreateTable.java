...
public static void parseCreateString(String createString) {
		...
		if(DavisBase.tableExists(tableName)){
			System.out.println("Table name"+tableName+" exists!!");
		}
		else
			{
...
/*
 * Create from DavisBase.java
 */
public static void createTable(String table, String[] col){                                     
	try{	
		/*
		 * Create table <table_name>.tbl
		 * page size = 512 bytes
		 */
		RandomAccessFile file_pointer = new RandomAccessFile("data/"+table+".tbl", "rw");
		file_pointer.setLength(Table.pageSize);										
		file_pointer.seek(0);				
		file_pointer.writeByte(0x0D);		
		file_pointer.close();
		
		file_pointer = new RandomAccessFile("data/davisbase_tables.tbl", "rw");
		
		int numOfPages = Table.pages(file_pointer);
		int page=1;
		for(int p = 1; p <= numOfPages; p++){
			int rm = Page.getRightMost(file_pointer, p);
			if(rm == 0)
				page = p;
		}
		...
		file_pointer = new RandomAccessFile("data/davisbase_columns.tbl", "rw");
		
		numOfPages = Table.pages(file_pointer);
		page=1;
		for(int p = 1; p <= numOfPages; p++){
			int rm = Page.getRightMost(file_pointer, p);
			if(rm == 0)
		...
		keys = Page.getKeyArray(file_pointer, page);
		l = keys[0];
		for(int i = 0; i < keys.length; i++)
			if(keys[i]>l)
				l = keys[i];
...
public static void insertInto(RandomAccessFile file, String table, String[] values){
	...
	if(page != 0)
		if(Page.hasKey(file, page, key)){
			System.out.println("Violation of UNIQUE constraint");
			return;
		}
	if(page == 0)
		page = 1;
